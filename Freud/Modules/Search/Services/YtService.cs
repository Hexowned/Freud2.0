...
{
    public class YtService : FreudHttpService
    {
        ...
        public YtService(string key)
        {
            ...
            if (!string.IsNullOrWhiteSpace(key))
            {
                ...
                {
                    ApiKey = key,
                    ApplicationName = Freud.ApplicationName
                });
            }
        }
        ...
        public async Task<IReadOnlyList<Page>> GetPaginatedResultsAsync(string query, int amount = 1, string type = null)
        {
            ...
            var results = await this.SearchAsync(query, amount, type);
        ...
        private async Task<SongInfo> GetSongInfoViaYtDlAsync(string url)
        {
            ...
            try
            {
                ...
                if (!TimeSpan.TryParseExact(data[4], new[] { "ss", "m\\:ss", "mm\\:ss",
                                                             "h\\:mm\\:ss", "hh\\:mm\\:ss",
                                                             "hhh\\:mm\\:ss" },
        ...
        {
            var request = this.yt.Search.List("snippet");
            request.Q = query;
