...
using Freud.Modules.Owner.Common;
using Humanizer;
...
namespace Freud.Modules.Owner
{
    ...
    [Description("Owner-only bot admin commands.")]
    [Aliases("admin", "adm")]
    [Cooldown(3, 5, CooldownBucketType.Global)]
    public partial class OwnerModule : FreudModule
    {
        public OwnerModule(SharedData shared, DatabaseContextBuilder dcb)
            : base(shared, dcb)
        {
        ...
        //
        [Command("announce"), NotBlocked, UsageInteractivity]
                                       ...
                                       [RemainingText, Description("Message to send.")] string message)
        {
            ...
            {
                Title = "An announcement from the team",
                Description = message,
                Color = DiscordColor.Red
            };

            var sb = new StringBuilder();
            foreach (var shard in Freud.ActiveShards)
            {
                foreach (var guild in shard.Client.Guilds.Values)
                {
                    ...
                    {
                        sb.AppendLine($"Warning: Failed to send a message to {guild.ToString()}");
                    }
                }
            }

            if (sb.Length > 0)
                await this.InformOfFailureAsync(ctx, $"Action finished with the following errors:\n\n{sb.ToString()}");
            else
                await this.InformAsync(ctx, StaticDiscordEmoji.Information, "Sent the message to all guilds!", important: false);
        }

        #endregion COMMAND_ANNOUNCE

        #region COMMAND_BOT_AVATAR

        //
        [Command("botavatar"), NotBlocked]
        [Description("Set bot avatar image.")]
                                           ...
                                           [Description("URL.")] Uri url)
        {
            ...
            {
                throw new CommandFailedException("Web exception thrown while fetching the image requested.", e);
            }

            await this.InformAsync(ctx, "Successfully changed the bot avatar!", important: false);
        }

        #endregion COMMAND_BOT_AVATAR

        #region COMMAND_BOT_NAME

        //
        [Command("botname"), NotBlocked]
        [Description("Set bot name.")]
        [Aliases("setbotname", "setname")]
        [UsageExampleArgs("Freud")]
        [RequireOwner]
        public async Task SetBotNameAsync(CommandContext ctx,
                                         [RemainingText, Description("New name.")] string name)
        {
            if (string.IsNullOrWhiteSpace(name))
                throw new InvalidCommandUsageException("Name is missing.");

            await ctx.Client.UpdateCurrentUserAsync(username: name);
            await this.InformAsync(ctx, $"Renamed the current bot username to {Formatter.Bold(ctx.Client.CurrentUser.Username)}");
        }

        #endregion COMMAND_BOT_NAME

        #region COMMAND_CLEAR_LOG

        //
        [Command("clearlog"), NotBlocked, UsageInteractivity]
        [Description("Clear the bot's logs")]
        ...
        public async Task ClearLogAsync(CommandContext ctx)
        {
            ...
            if (!this.Shared.LogProvider.ClearLog())
                throw new CommandFailedException("Failed to delete the log file(s)!");

            await this.InformAsync(ctx, $"Logs have been cleared!", important: false);
        }

        #endregion COMMAND_CLEAR_LOG

        #region COMMAND_FILE_LOG

        //
        [Command("filelong"), NotBlocked]
        [Description("Toggle writing to a log file.")]
        [Aliases("setf1", "fk", "setfilelog")]
        ...
        public async Task LogAsync(CommandContext ctx,
                                  [Description("Bypass current configuration and search for the file anyway?")] bool bypass = false)
        {
            if (!bypass && this.Shared.BotConfiguration.LogToFile)
                throw new CommandFailedException("Logs weren't dumped to any files.");
        ...
        #endregion COMMAND_LOG

        #region COMMAND_DB_QUERY

        //
        [Command("dbquery"), NotBlocked, Priority(0)]
        [Description("Execute SQL query on the bots database.")]
        [Aliases("sql", "dbq", "q")]
        [UsageExampleArgs("SELECT * FROM f.msgcount;")]
        [RequireOwner]
        public async Task DatabaseQuery(CommandContext ctx,
                                       [RemainingText, Description("SQL Query")] string query)
        {
            if (string.IsNullOrWhiteSpace(query))
                throw new InvalidCommandUsageException("The Query is missing.");

            var res = new List<IReadOnlyDictionary<string, string>>();
            using (var dc = this.Database.CreateContext())
            using (var dr = await dc.Database.ExecuteSqlQueryAsync(query))
            {
            ...
            {
                await this.InformAsync(ctx, StaticDiscordEmoji.Information, "No results have been returned (this is alright if your query wasn't a SELECT query).");
                return;
            }

            int maxlen = 1 + res.First().Select(r => r.Key).OrderByDescending(r => r.Length).First().Length;
        ...
        public async Task DatabaseQuery(CommandContext ctx)
        {
            ...
            var attachment = ctx.Message.Attachments.FirstOrDefault(att => att.FileName.EndsWith(".sql"));
            if (attachment is null)
                throw new CommandFailedException("No .sql files were attached!");
        ...
        #endregion COMMAND_DB_QUERY

        #region COMMAND_EVALUATION_C#

        //
        // TODO: Implement python as well as other languages to create an informative bot that can evaluate code and be productive tool for discord
                                       ...
                                       [RemainingText, Description("Code to evaluate")] string code)
        {
            if (string.IsNullOrWhiteSpace(code))
                throw new InvalidCommandUsageException("Code was not found!");
            ...
            var emb = new DiscordEmbedBuilder
            {
                Title = "Evaluating input...",
                Color = this.ModuleColor
            };

            var msg = await ctx.RespondAsync(embed: emb.Build());
            var globals = new EvaluationEnvironment(ctx);
            var sopts = ScriptOptions.Default.WithImports("System", "System.Collections.Generic", "System.Linq", "System.Net.Http",
                                                          "System.Net.Http.Headers", "System.Reflection", "System.Text",
                                                          "System.Text.RegularExpressions", "System.Threading.Tasks", "DSharpPlus",
                                                          "DSharpPlus.CommandsNext", "DSharpPlus.Entities", "DSharpPlus.Interactivity")
            ...
            if (diag.Any(d => d.Severity == DiagnosticSeverity.Error))
            {
                ...
                {
                    Title = "Compilation fialed",
                    Description = $"Compilation failed after {sw1.ElapsedMilliseconds.ToString("#,##0")}ms with {diag.Length} errors.",
                    Color = DiscordColor.Red
                };

                foreach (var d in diag.Take(3))
                {
                    var flps = d.Location.GetLineSpan();
                    emb.AddField($"Error at line: {flps.StartLinePosition.Line}, {flps.StartLinePosition.Character}", Formatter.InlineCode(d.GetMessage()));
                }
        ...
        #endregion COMMAND_EVALUATION_C#

        #region COMMAND_LEAVE_GUILDS

        //
        [Command("leaveguilds"), NotBlocked]
        [Description("Leaves the given guilds.")]
        [Aliases("leave", "gtfo")]
        [UsageExampleArgs("337570344149975050")]
        [RequireOwner]
                                          ...
                                          [Description("Guild ID list.")] params ulong[] gids)
        {
            ...
            foreach (ulong gid in gids)
            {
                try
                {
                    if (ctx.Client.Guilds.TryGetValue(gid, out var guild))
                        await guild.LeaveAsync();
                } catch
                {
                    sb.AppendLine($"Error: Failed to leave the guild with the ID: {Formatter.InlineCode(gid.ToString())}!");
                }
            }
            ...
            if (gids.All(gid => gid != ctx.Guild.Id))
            {
                if (sb.Length > 0)
                    ...
                    await this.InformAsync(ctx, StaticDiscordEmoji.Information, "Successfully left all of the given guilds!", important: false);
            }
        }

        #endregion COMMAND_LEAVE_GUILDS

        #region COMMAND_SEND_MESSAGE
                                   ...
                                   [RemainingText, Description("Message.")] string message)
        {
            ...
            if (desc == "u")
            {
                ...
                if (dm is null)
                    throw new CommandFailedException("I can't talk to that user... probably blocked me");
        ...
        #endregion COMMAND_SEND_MESSAGE

        #region COMMAND_SHUTDOWN

        //
        [Command("shutdown"), NotBlocked, Priority(1)]
        ...
        #endregion COMMAND_SUDO

        #region COMMAND_TOGGLE_IGNORE

        //
        [Command("toggleignore")]
        [Description("Toggle bots reaction to commands.")]
        [Aliases("ti")]
        [RequireOwner]
        ...
        #endregion COMMAND_TOGGLE_IGNORE
        ...
        public Task UpdateAsync(CommandContext ctx)
        {
            ...
            {
                throw new CommandFailedException("Cannot determine the host OS (mac is not supported at this time)!");
            }
        ...
        #region COMMAND_GENERATE_COMMANDS
                                                  ...
                                                  [RemainingText, Description("File path")] string path = null)
        {
            ...
            var modules = commands.GroupBy(c => ModuleAttribute.ForCommand(c)).OrderBy(g => g.Key.Module).ToDictionary(g => g.Key, g => g.OrderBy(c => c.QualifiedName).ToList());

            foreach ((var mattr, var cmdlist) in modules)
            {
                ...
                foreach (var cmd in cmdlist)
                {
                    ...
                    sb.AppendLine(Formatter.Italic(cmd.Description ?? "No description provided for this command")).AppendLine();

                    var execChecks = cmd.ExecutionChecks.AsEnumerable();
                    ...
                    var perms = execChecks.Where(chk => chk is RequirePermissionsAttribute)
                                          .Select(chk => chk as RequirePermissionsAttribute)
                                          .Select(chk => chk.Permissions.ToPermissionString())
                                          .Union(execChecks.Where(chk => chk is RequireOwnerOrPermissionsAttribute)
                                                           .Select(chk => chk as RequireOwnerOrPermissionsAttribute)
                                                           .Select(chk => chk.Permissions.ToPermissionString()));

                    var uperms = execChecks.Where(chk => chk is RequireUserPermissionsAttribute)
                                           .Select(chk => chk as RequireUserPermissionsAttribute)
                                           .Select(chk => chk.Permissions.ToPermissionString());

                    var bperms = execChecks.Where(chk => chk is RequireBotPermissionsAttribute)
                ...
                {
                    throw new CommandFailedException($"IOException occured while saving {filename}!", e);
                }
            ...
            {
                throw new CommandFailedException($"IOException occured while saving the main file!", e);
            }

            await this.InformAsync(ctx, $"Command list created at: {Formatter.InlineCode(current.FullName)}!", important: false);
        }

        #endregion COMMAND_GENERATE_COMMANDS
    }
}
