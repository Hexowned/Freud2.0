...
namespace Freud.Common
{
    public sealed class CustomHelpFormatter : BaseHelpFormatter
    {
            ...
            : base(ctx)
        {
            ...
            this.emb.WithFooter("Detailed documentation @ https://github.com/nexowned/Freud2.0", ctx.Client.CurrentUser.AvatarUrl);
        }
        ...
        public override BaseHelpFormatter WithCommand(Command cmd)
        {
            ...
            var perms = checks.Where(chk => chk is RequirePermissionsAttribute)
                              .Select(chk => chk as RequirePermissionsAttribute)
                              .Select(chk => chk.Permissions.ToPermissionString())
                              .Union(checks.Where(chk => chk is RequireOwnerOrPermissionsAttribute)
            ...
            if (checks.Any(chk => chk is RequirePrivilegedUserAttribute))
                sb.AppendLine(Formatter.Bold("Privileged users only."));
            if (perms.Any())
                sb.AppendLine(Formatter.InlineCode(string.Join(", ", perms)));
            if (uperms.Any())
                sb.Append(Formatter.Bold("User permissions: ")).AppendLine(Formatter.InlineCode(string.Join(", ", uperms)));
            if (bperms.Any())
                sb.Append(Formatter.Bold("Bot permissions: ")).AppendLine(Formatter.InlineCode(string.Join(", ", bperms)));
