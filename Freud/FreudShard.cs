...
namespace Freud
{
    public sealed class FreudShard
    {
        ...
        {
            Commands = cnext.GetAllRegisteredCommands().Where(cmd => cmd.Parent is null).SelectMany(cmd => cmd.Aliases.Select(alias => (alias, cmd)).Concat(new[] { (cmd.Name, cmd) })).ToList();
        }
        ...
        public FreudShard(int sid, DatabaseContextBuilder dcb, SharedData shared)
        {
            this.Id = sid;
            this.Database = dcb;
        ...
        private void SetupCommands()
        {
            this.CNext = this.Client.UseCommandsNext(new CommandsNextConfiguration
            {
                ...
                PrefixResolver = this.PrefixResolverAsyc,
        ...
        private Task<int> PrefixResolverAsyc(DiscordMessage m)
        {
